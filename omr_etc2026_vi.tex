...
được con người rà soát, trên sáu thiết bị Web, với mốc Native chạy trên hai
trong số đó. Chúng tôi báo cáo độ chính
...
\begin{enumerate}[leftmargin=*, itemsep=1pt, topsep=2pt]
  ...
  phép đo độ chính xác có giám sát, độ trễ và tài nguyên trên sáu thiết bị
  Web Edge và một mốc Native Android trên hai trong số đó.
...
Sáu thiết bị Web phát lại cùng bộ $N{=}185$ ảnh: một máy desktop tham
chiếu (i5-14600K / RTX 4070 Ti SUPER), một ASUS Vivobook (i5-13500H), một
MacBook Air M1, một iPhone~16 (Safari/WebKit), một máy tính bảng Android
Poco Pad M1 (Chrome/Blink), và chiếc Redmi Note~13 Pro+ (Chrome/Blink); mốc
Native Android chạy trên hai trong số đó, Poco Pad và Redmi. Các hàng độ
trễ Web báo cáo \texttt{cpp\_ms} phía
...
Độ trễ hệ lai Web & \texttt{runs/batch\_results/} & tổng worker + \texttt{yolo\_ms} \\
...
corner Native nhận dạng đúng $99,44\%$ câu trên cả hai thiết bị Native
(Redmi và Poco Pad), ngang với hệ lai Web ($99,45\%$) và đúng trên cả năm
tập con kể cả các phiếu tối, với chi phí suy luận $335$\,ms dưới NNAPI và
$530$\,ms trên bốn luồng CPU trên Redmi. So sánh tương đương,
...
chứ không chỉ cộng thêm vào. Bảng~\ref{tab:latency} liệt kê \texttt{cpp\_ms}
mỗi phiếu cho cả hai pipeline trên sáu thiết bị; trên ba thiết bị đo theo
giao thức điểm-góc, mỗi cái là trung vị năm lần chạy, hai hiệu ứng lặp lại.
Thứ nhất, hai pipeline đạt cùng độ chính
...
\begin{table}[t]
\caption{Độ trễ lõi C\texttt{++} mỗi phiếu (\texttt{cpp\_ms}), chỉ-CV so với
hệ lai theo điểm-góc, trên sáu thiết bị Web.}
\label{tab:latency}
\centering
\setlength{\tabcolsep}{6pt}
\footnotesize
...
\vspace{2pt}
\parbox{\columnwidth}{\footnotesize
\texttt{cpp\_ms} là lõi OMR C\texttt{++}. Hệ lai chạy thêm bộ phát hiện
corner trên WebGPU, mà suy luận cộng thêm $\approx140$\,ms (laptop) tới
$\approx550$\,ms (điện thoại) và được tính vào tổng thời gian worker ở
Hình~\ref{fig:latency}. Ba thiết bị đầu là trung vị năm lần chạy theo giao
thức điểm-góc ($N{=}185$); Desktop, Mac và iPhone từ một vòng đo trước. Độ
chính xác đồng nhất theo thiết bị: CV 99,03\%, hệ lai 99,45\%
(Bảng~\ref{tab:accuracy}).}
\end{table}
...
\begin{enumerate}[label=(\roman*), leftmargin=*, itemsep=1pt, topsep=2pt]
  ...
  Bảng~\ref{tab:latency} được báo cáo dưới dạng trung vị năm lần chạy, và so
