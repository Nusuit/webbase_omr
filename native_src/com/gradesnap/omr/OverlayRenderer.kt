...
object OverlayRenderer {
    ...
    // ── Paint cho chế độ "phát hiện" (detect) — xanh dương ──────────────────────
    private val paintDetect = Paint().apply {
    ...
    private val paintDetectFill = Paint().apply {
        color = Color.parseColor("#332196F3")   // xanh dương mờ
        style = Paint.Style.FILL
        isAntiAlias = true
    }

    private val paintUndetected = Paint().apply {   // badge "chưa đọc được"
        color = Color.parseColor("#F44336")
        style = Paint.Style.FILL
        isAntiAlias = true
    }

    private val paintUndetectedText = Paint().apply {
        color = Color.WHITE
        textSize = 22f
        typeface = Typeface.DEFAULT_BOLD
        isAntiAlias = true
    ...
    /**
     * Chế độ "phát hiện": vẽ XANH DƯƠNG các bubble model detect được — KHÔNG so đáp án.
     * Dùng cho preview đáp án (Bước 1) và preview bài làm (tab "Phát hiện").
     *
     * @param detected    qNum → tập option detect được (vd A, B…)
     * @param suspicious  câu nghi vấn (đa lựa chọn / tô mờ) → badge cam "?"
     * @param undetected  câu không đọc được gì → badge đỏ "✗"
     */
    fun renderDetection(
        warped: Bitmap,
        config: OmrConfig,
        detected: Map<Int, Set<String>>,
        suspicious: Set<Int>,
        undetected: Set<Int>
    ): Bitmap {
        val out = warped.copy(Bitmap.Config.ARGB_8888, true)
        ...
        for (block in config.questions.blocks) {
            ...
            for (row in 0 until block.rows) {
                val qNum = block.questionStart + row
                val filled = detected[qNum].orEmpty()

                for (col in 0 until block.cols) {
                    val opt = options.getOrNull(col) ?: continue
                    if (opt !in filled) continue
                    val cx = block.x1 + col * cellW + cellW / 2
                    val cy = block.y1 + row * cellH + cellH / 2
                    val r  = (minOf(cellW, cellH) * 0.38f)
                    canvas.drawCircle(cx, cy, r, paintDetectFill)
                    canvas.drawCircle(cx, cy, r, paintDetect)
                }
                ...
                when {
                    qNum in undetected -> {
                        canvas.drawCircle(badgeCx, badgeCy, 12f, paintUndetected)
                        canvas.drawText("✗", badgeCx - 7f, badgeCy + 7f, paintUndetectedText)
                    }
                    qNum in suspicious -> {
                        canvas.drawCircle(badgeCx, badgeCy, 10f, paintSuspicious)
                        canvas.drawText("?", badgeCx - 6f, badgeCy + 8f, paintText)
