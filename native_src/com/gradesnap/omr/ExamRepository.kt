...
class ExamRepository(private val context: Context) {
    ...
    fun loadProject(id: String): ExamProject? {
        ...
        return runCatching { gson.fromJson(target.readText(), ExamProject::class.java) }
            .getOrNull()
    }
    ...
    /** Chỉ các đáp án đã được người dùng xác nhận làm ground truth (confirmed == true). */
    fun loadConfirmedKeys(projectId: String): Map<String, AnswerKey> =
        loadAllAnswerKeys(projectId).filterValues { it.confirmed }
    ...
    fun loadAllStudentResults(projectId: String): List<StudentResult> {
        ...
        val type = object : TypeToken<StudentResult>() {}.type
        return dir.listFiles { f -> f.extension == "json" }
            ?.mapNotNull { f -> runCatching { gson.fromJson<StudentResult>(f.readText(), type) }.getOrNull() }
