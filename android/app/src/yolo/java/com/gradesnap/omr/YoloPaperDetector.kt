...
class YoloPaperDetector private constructor(context: Context) : PaperDetector {
    ...
    companion object {
        ...
        private const val EXPAND      = 0.20f  // 20% bbox expansion
    ...
    init {
        ...
        Log.i(TAG, "ORT version : ${OrtEnvironment.getVersion()}")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1) { // API 27
            // Try FP16 first (best throughput on GPU/APU)
            ns = createNnapiSession(modelBytes, fp16 = true)
            if (ns != null) {
                nAvail = true; nnapiUseFp16 = true
                Log.i(TAG, "NNAPI session: ready — FP16 enabled")
                Log.i(TAG, "NOTE: check logcat for 'NnapiExecutionProvider' to see GPU/DSP/CPU assignment")
            } else {
                // Retry FP32 (no accuracy drift)
                ns = createNnapiSession(modelBytes, fp16 = false)
                if (ns != null) {
                    nAvail = true; nnapiUseFp16 = false
                    Log.i(TAG, "NNAPI session: ready — FP32 only (FP16 rejected by driver)")
                } else {
    ...
    private fun createNnapiSession(modelBytes: ByteArray, fp16: Boolean): OrtSession? = try {
        val opts = OrtSession.SessionOptions()
        if (fp16) {
            // NNAPIFlags is nested in OrtSession.SessionOptions in ORT 1.x.
            // If compile error → remove the EnumSet block and use opts.addNnapi().
            @Suppress("UNCHECKED_CAST")
            opts.addNnapi(EnumSet.of(OrtSession.SessionOptions.NNAPIFlags.USE_FP16))
        } else {
            opts.addNnapi()
        }
        env.createSession(modelBytes, opts)
    } catch (e: Exception) {
        Log.d(TAG, "createNnapiSession(fp16=$fp16) failed: ${e.javaClass.simpleName}: ${e.message}")
        null
    }
    ...
    private fun inferAndWarpInternal(img: Mat, session: OrtSession, tag: String): Mat {
        ...
        Log.i(TAG, "[$tag] prep=${prepMs}ms  ort=${lastOrtMs}ms  stage=${lastStageMs}ms" +
